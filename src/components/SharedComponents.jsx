...
export const Pill = ({ children, color = "accent" }) => {
    ...
    return (
        <span className={`inline-flex items-center px-2.5 py-1.5 rounded-full text-[10px] font-bold tracking-wider uppercase leading-none ${map[color]}`}>
            {children}
        </span>
    );
};
...
export const Avatar = ({ name, size = 36 }) => {
    ...
    return (
        <div className="rounded-full flex items-center justify-center font-bold text-white flex-shrink-0 select-none ring-2 ring-white/50 shadow-sm"
...
export const Card = ({ children, className = "" }) => (
    <div className={`glass-card rounded-[24px] ${className} hover:fluent-shadow transition-shadow duration-300`}>
        {children}
    </div>
);
...
export const SectionTitle = ({ children, sub }) => (
    <div className="mb-6">
        <h2 className="text-base font-bold text-slate-800 tracking-tight">{children}</h2>
